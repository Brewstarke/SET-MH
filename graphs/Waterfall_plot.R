...
##
#
# Attempt at recreating a 'waterfall' chart with our SET-MH data sumamries
# Note that these data are not 100% correct in that the shallow accretion sample window is not exactly comparable to the elevation readings. 
# The shallow accretion rates are taken from the full time series

# From https://gist.github.com/jkeirstead/df85c839bd8f0026ee05#file-waterfall-r
# Special thanks to James Keirstead @ http://www.jameskeirstead.ca/blog/waterfall-plots-in-r/
#----
...
#----
# Arrange the summaryTable_Wide into proper format and create 'values'

waterfall_df <- SummaryTable_wide %>%
	select(Site_Name, ends_with('_change'),ends_with('_Rate'), Stratafication) %>%
	filter(Site_Name == "Pine Neck", Stratafication == 'Low Marsh') %>%
	gather(category, value, -Site_Name, -Stratafication) %>%
	select(-Stratafication) %>%
	dplyr::rename(sector = Site_Name)
waterfall_df$sector <- as.character(waterfall_df$sector)
