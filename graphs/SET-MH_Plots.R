...
SETMH_Plot_overlayvert <- function(plot_title)
{
	require(ggplot2)
	require(ggthemes)
	require(plyr)
	...
	data <- SET.data.Melt[(SET.data.Melt$SET_Type == "Rod SET"),] # Subset the data type - removed to add faceting on this variable
	#data <- join(x = SET.data.Melt, y= SA.data.M, by = c("Site_Name", "Stratafication", "Plot_Name", "Start_Date"))
	data_01 <- SA.data.M
...
SETMH_Plot_overlayvert(plot_title = "Marsh Elevation Changes Across LI")
...
SET_MH_Plot_smooth <- function(plot_title)
{
	...
	plot_title <- plot_title
	data <- SET.data.Melt %>% filter(Site_Name == "Mashomack Point" |Site_Name == "Indian Island" | Site_Name =="Pine Neck")
	...
	SiteOrder <- unique(SET.data.Melt$Site_Name)[c(9,8,1:2, 4, 6, 7, 3, 5)]
	data$Site_Name <- factor(data$Site_Name, levels= SiteOrder) 
# 	data_02 <- SET.data.Melt %>% filter(SET_Type == "Shallow SET" & Site_Name == "Indian Island" | SET_Type == "Shallow SET" & Site_Name == "Pine Neck")
# 	
	# Add control structure to ensure Site is available to select.
	#data$highlight <-  as.factor(data$Site_Name != highlightSite)
...
SET_MH_Plot_line <- function(plot_title)
{
	...
	plot_title <- plot_title
	data <- SET.data.Melt[(SET.data.Melt$SET_Type == "Rod SET" & SET.data.Melt$Site_Name == "Pine Neck" | SET.data.Melt$Site_Name == "Pine Neck"),] # Subset the data type - removed to add faceting on this variable
	#data <- join(x = SET.data.Melt, y= SA.data.M, by = c("Site_Name", "Stratafication", "Plot_Name", "Start_Date"))
	...
	SiteOrder <- unique(SET.data.Melt$Site_Name)[c(9,8,1:2, 4, 6, 7, 3, 5)]
	data$Site_Name <- factor(data$Site_Name, levels= SiteOrder) 
	data_02 <- SET.data.Melt %>% filter(SET_Type == "Shallow SET" & Site_Name == "Pine Neck" | SET_Type == "Shallow SET" & Site_Name == "Pine Neck")
...
SET_MH_Plot_line_all <- function(plot_title)
{
	...
	SET_Plot2A <- SET_Plot2 + 		
		stat_summary(data= data, # SET data
			 aes(y = change,
			 		x = as.Date(Date),
			 		color= interaction(SET_Type, Stratafication, sep = " ")),
# 					group= interaction(levels, sep= " ")),
		...
		stat_summary(data= data_02, # Shallow SET data
								 aes(y = change,
								 		x = as.Date(Date),
								 		#color= interaction(Site_Name, Stratafication, SET_Type, sep = " ")),
 								 		group= interaction(SET_Type, Stratafication, sep = " ")),
								 geom= "line",
								 fun.y = "mean",
								 se = TRUE, 
								 size = 1,
		...
		facet_wrap( ~ Site_Name, ncol = 3) +
		scale_x_date(breaks = "6 months", labels=date_format("%Y"),limits = as.Date(c('2008-05-01','2015-12-01'))) +
		theme(legend.position= "top") +	
		theme_bw(base_family="serif") + 
